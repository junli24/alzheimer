...
def extract_surface_area(area_gii_path, atlas_gii_path, num_rois, hemi_prefix='lh', start_label=1):
    """
...
if __name__ == '__main__':
    ...
    # 假设文件路径
    AREA_LH_GII = f'{SUBJECT_ID}_space-fsaverage5_hemi-L.area.gii'
    ...
    # 检查文件是否存在
    if not os.path.exists(AREA_LH_GII) or not os.path.exists(ATLAS_LH_GII):
        print("错误: 找不到文件。请检查以下路径是否正确：")
        print(AREA_LH_GII)
        print(ATLAS_LH_GII)
    else:
        # 运行提取函数
        lh_area_features = extract_surface_area(
            AREA_LH_GII,
            ATLAS_LH_GII,
            LH_NUM_ROIS,
            hemi_prefix='lh',
        ...
        # 保存为 CSV 文件 (F1)
        output_filename = f'{SUBJECT_ID}_LH_Area_Schaefer200.csv'
