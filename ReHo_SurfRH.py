...
def extract_surface_metric(metric_gii_path, atlas_gii_path, num_rois, metric_name, hemi_prefix='rh', start_label=1):
    """
    ...
    for i in range(start_label, end_label + 1): 
        ...
        # 排除标签 0（背景）
        if len(vertex_indices) == 0 and i != 0:
            # 这种情况通常只发生在图谱有缺陷时
            continue
...
if __name__ == '__main__':
    ...
    # 假设文件路径
    REHO_RH_GII = f'ReHo_rh_{SUBJECT_ID}.func.gii'
    ...
    # 检查文件是否存在
    if not os.path.exists(REHO_RH_GII) or not os.path.exists(ATLAS_RH_GII):
        print("错误: 找不到文件。请检查以下路径是否正确：")
        print(REHO_RH_GII)
        print(ATLAS_RH_GII)
    else:
        # 运行提取函数
        rh_reho_features = extract_surface_metric(
            REHO_RH_GII,
            ATLAS_RH_GII,
            RH_NUM_ROIS,
            METRIC_NAME,
        ...
        print(f"\n--- 右脑皮层 {METRIC_NAME} 提取结果 (部分) ---")
        print(results_df.head())
        
        # 保存为 CSV 文件
        output_filename = f'{SUBJECT_ID}_RH_{METRIC_NAME}_Schaefer200.csv'
