...
def extract_surface_curvature(curv_gii_path, atlas_gii_path, num_rois, hemi_prefix='lh', start_label=1):
    """
    使用 NiBabel 从皮层曲率 GIIfi 文件中，根据 Schaefer 表面图谱提取 ROI 平均曲率 (Mean Curvature)。

    Args:
        curv_gii_path (str): 皮层曲率数据的 GIIfi 文件路径。
    ...
    Returns:
        numpy.ndarray: 包含 ROI 平均曲率的向量 (num_rois, 1)。
    """
    
    METRIC_NAME = 'Mean_Curvature'
    print(f"--- 正在处理 {hemi_prefix.upper()} 皮层 {METRIC_NAME} (求平均) ---")

    # 1. 加载 曲率数据
    try:
        curv_gii = nib.load(curv_gii_path)
        # 曲率数据通常是每个顶点对应的数值
        curv_data = curv_gii.darrays[0].data
        print(f"成功加载 Curvature 数据，维度: {curv_data.shape}")
    except Exception as e:
        print(f"错误: 无法加载曲率文件 {curv_gii_path}. 确保文件存在且格式正确。")
        raise e
    ...
    if curv_data.shape[0] != atlas_labels.shape[0]:
        raise ValueError(
            "曲率数据和 Atlas 标签的顶点数量不一致。请检查文件是否来自同一 fsaverage5 表面。"
        )

    # 3. 提取 ROI 平均曲率
    mean_curv_values = np.zeros(num_rois)
    ...
    for i in range(start_label, end_label + 1): 
        ...
        # 提取这些顶点对应的曲率值
        roi_curv_data = curv_data[vertex_indices]
        
        # ***** 关键操作：这里使用 np.mean() 求平均 *****
        mean_curv = np.mean(roi_curv_data)
        
        # 存储结果 (使用 i - start_label 作为 Python 索引 0 到 199)
        mean_curv_values[i - start_label] = mean_curv
        
        if (i - start_label + 1) % 50 == 0:
            print(f"已处理 ROI {i - start_label + 1}/{num_rois}")

    print("--- 提取完成 ---")
    return mean_curv_values.reshape(-1, 1) # 返回 (200, 1) 的列向量
...
if __name__ == '__main__':
    ...
    # 假设文件路径
    CURV_LH_GII = f'{SUBJECT_ID}_space-fsaverage5_hemi-L.curv.gii'
    ...
    # 检查文件是否存在
    if not os.path.exists(CURV_LH_GII) or not os.path.exists(ATLAS_LH_GII):
        print("错误: 找不到文件。请检查以下路径是否正确：")
        print(CURV_LH_GII)
        print(ATLAS_LH_GII)
    else:
        # 运行提取函数
        lh_curv_features = extract_surface_curvature(
            CURV_LH_GII,
            ATLAS_LH_GII,
            LH_NUM_ROIS,
            hemi_prefix='lh',
        ...
        results_df = pd.DataFrame(
            lh_curv_features, 
            index=roi_names, 
            columns=[f'MeanCurv_{SUBJECT_ID}']
        )
        ...
        # 保存为 CSV 文件 (F2)
        output_filename = f'{SUBJECT_ID}_LH_MeanCurv_Schaefer200.csv'
