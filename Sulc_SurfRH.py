...
def extract_surface_sulc(sulc_gii_path, atlas_gii_path, num_rois, hemi_prefix='rh', start_label=1):
    """
...
if __name__ == '__main__':
    ...
    # 假设文件路径
    SULC_RH_GII = f'{SUBJECT_ID}_space-fsaverage5_hemi-R.sulc.gii'
    ...
    # 检查文件是否存在
    if not os.path.exists(SULC_RH_GII) or not os.path.exists(ATLAS_RH_GII):
        print("错误: 找不到文件。请检查以下路径是否正确：")
        print(SULC_RH_GII)
        print(ATLAS_RH_GII)
    else:
        # 运行提取函数
        rh_sulc_features = extract_surface_sulc(
            SULC_RH_GII,
            ATLAS_RH_GII,
            RH_NUM_ROIS,
            hemi_prefix='rh',
        ...
        # 保存为 CSV 文件 (F3)
        output_filename = f'{SUBJECT_ID}_RH_MeanSulc_Schaefer200.csv'
